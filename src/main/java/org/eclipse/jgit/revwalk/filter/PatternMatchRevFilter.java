...
public abstract class PatternMatchRevFilter extends RevFilter {
     ...
     *
     * @param patternText original pattern string supplied by the user or the
     *                    application.
     ...
     *
     * @param pattern     text of the pattern. Callers may want to surround their
     *                    pattern with ".*" on either end to allow matching in the
     *                    middle of the string.
     * @param innerString should .* be wrapped around the pattern of ^ and $ are
     *                    missing? Most users will want this set.
     * @param rawEncoding should {@link #forceToRaw(String)} be applied to the pattern
     *                    before compiling it?
     * @param flags       flags from {@link Pattern} to control how
     *                    matching performs.
     */
