...
public class RevCommit extends RevObject {
     ...
     *
     * @param rw  the revision pool to allocate the commit within. The commit's
     *            tree and parent pointers will be obtained from this pool.
     * @param raw the canonical formatted commit to be parsed. This buffer will
     ...
     * necessary.
     *
     * @param flag the single flag value to carry back onto parents.
     */
     ...
     *
     * @param nth parent index to obtain. Must be in the range 0 through
     *            {@link #getParentCount()}-1.
     * @return the specified parent.
     * @throws ArrayIndexOutOfBoundsException an invalid parent index was specified.
     */
     ...
     *
     * @param keyName footer key to find values of, case-insensitive.
     ...
     * Get the values of all footer lines with the given key.
     *
     * @param key footer key to find values of, case-insensitive.
     * @return values of footers with key of {@code keyName}, ordered by their
     * order of appearance. Duplicates may be returned if the same
     * footer appeared more than once. Empty list if no footers appear
     * with the specified key, or there are no footers at all.
