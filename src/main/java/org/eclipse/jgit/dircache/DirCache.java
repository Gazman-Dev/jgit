...
public class DirCache {
     ...
     *
     * @param reader reader to access the tree objects from a repository.
     * @param treeId tree to read. Must identify a tree, not a tree-ish.
     * @return a new cache which has no backing store file, but contains the
     * contents of {@code treeId}.
     * @throws IOException one or more trees not available from the ObjectReader.
     * @since 4.2
     */
     ...
     *
     * @throws IOException            the index file is present but could not be read. This
     ...
     * by someone else.
     * @throws IOException the output file could not be created. The caller does not
     *                     hold the lock.
     */
     ...
     *
     * @throws IOException the output file could not be created. The caller no longer
     *                     holds the lock.
     */
     ...
     *
     * @param p    the byte array starting with the path to search for.
     * @param pLen the length of the path in bytes
     * @return if &gt;= 0 then the return value is the position of the entry in
     * the index; pass to {@link #getEntry(int)} to obtain the entry
     * information. If &lt; 0 the entry does not exist in the index.
     * @since 3.4
     ...
     *
     * @param position entry position of the path that should be skipped.
     ...
     *
     * @param i position of the entry to get.
     ...
     *
     * @param build if true and the cache tree is not present in the index it will
     ...
     *
     * @param ow the writer to use when serializing to the store. The caller is
     *           responsible for flushing the inserter before trying to use the
     *           returned tree identity.
     * @return identity for the root tree.
     * @throws UnmergedPathException one or more paths contain higher-order stages (stage &gt; 0),
     *                               which cannot be stored in a tree object.
     * @throws IllegalStateException one or more paths contain an invalid mode which should never
     *                               appear in a tree object.
     * @throws IOException           an unexpected error occurred writing to the object store.
     */
