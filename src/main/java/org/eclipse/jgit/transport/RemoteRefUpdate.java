 ...
 * {@link PushProcess} class, being read-only for
 * client.
 * <p>
 * Client can create instances of this class directly, basing on user
 * specification and advertised refs
 * ({@link Connection} or through
 * {@link Transport} helper methods. Apply this
 * specification on remote repository using
 * {@link Transport#push(org.eclipse.jgit.lib.ProgressMonitor, Collection)}
 * method.
 * </p>
 */
public class RemoteRefUpdate {
     ...
     *
     * @param localDb             local repository to push from.
     * @param srcRef              source revision - any string resolvable by
     ...
     *                            delete request.
     * @param remoteName          full name of a remote ref to update, e.g. "refs/heads/master"
     *                            (no wildcard, no short name).
     * @param forceUpdate         true when caller want remote ref to be updated regardless
     *                            whether it is fast-forward update (old object is ancestor of
     *                            new object).
     * @param localName           optional full name of a local stored tracking branch, to
     *                            update after push, e.g. "refs/remotes/zawir/dirty" (no
     *                            wildcard, no short name); null if no local tracking branch
     *                            should be updated.
     * @param expectedOldObjectId optional object id that caller is expecting, requiring to be
     ...
     * @throws IOException              when I/O error occurred during creating
     *                                  {@link TrackingRefUpdate} for
     *                                  local tracking branch or srcRef can't be resolved to any
     *                                  object.
     * @throws IllegalArgumentException if some required parameter was null
     */
     ...
     *
     * @param localDb             local repository to push from.
     * @param srcRef              source revision. Use null to delete.
     * @param remoteName          full name of a remote ref to update, e.g. "refs/heads/master"
     *                            (no wildcard, no short name).
     * @param forceUpdate         true when caller want remote ref to be updated regardless
     *                            whether it is fast-forward update (old object is ancestor of
     ...
     *
     * @param localDb             local repository to push from.
     * @param srcRef              source revision to label srcId with. If null srcId.name() will
     *                            be used instead.
     * @param srcId               The new object that the caller wants remote ref to be after
     *                            update. Use null or
     ...
     *
     * @param base                   configuration base.
     * @param newExpectedOldObjectId new expected object id value.
     * @throws IOException when I/O error occurred during creating
     *                     {@link TrackingRefUpdate} for
     ...
     *
     * @param localDb     local repository to push from
     * @param forceUpdate whether non-fast-forward updates shall be allowed
     * @param fetchSpecs  The fetch {@link RefSpec}s to use when this placeholder is
     ...
     *
     * @param walk walker used for checking update properties.
     * @throws IOException when I/O error occurred during update
     */
