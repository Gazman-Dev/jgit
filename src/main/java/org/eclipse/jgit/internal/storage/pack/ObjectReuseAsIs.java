...
public interface ObjectReuseAsIs {
     ...
     *
     * @param objectId the id of the object that will be packed.
     * @param type     the Git type of the object that will be packed.
     ...
     *
     * @param packer  the packer that will write the object in the near future.
     * @param monitor progress monitor, implementation should update the monitor
     *                once for each item in the iteration when selection is done.
     * @param objects the objects that are being packed.
     * @throws MissingObjectException there is no representation available for the object, as it is
     *                                no longer in the repository. Packing will abort.
     * @throws IOException            the repository cannot be accessed. Packing will abort.
     */
     ...
     * for reuse.
     *
     * @param out  the stream to write each object to.
     * @param list the list of objects to write. Objects should be written in
     *             approximately this order. Implementors may resort the list
     *             elements in-place during writing if desired.
     * @throws IOException the stream cannot be written to, or one or more required
     ...
     * </pre>
     *
     * @param out      stream the object should be written to.
     * @param otp      the object's saved representation information.
     * @param validate if true the representation must be validated and not be
     *                 corrupt before being reused. If false, validation may be
     *                 skipped as it will be performed elsewhere in the processing
     *                 pipeline.
     * @throws StoredObjectRepresentationNotAvailableException the previously selected representation is no longer
     ...
     *                                                         packing will abort.
     * @throws IOException                                     the stream's write method threw an exception. Packing will
     *                                                         abort.
     */
     ...
     *
     * @param out  stream to append the pack onto.
     * @param pack the cached pack to send.
     * @throws IOException the pack cannot be read, or stream did not accept a write.
     */
     ...
     *
     * @param needBitmap the bitmap that contains all of the objects the client wants.
     * @return the available cached packs.
     * @throws IOException the cached packs cannot be listed from the repository.
