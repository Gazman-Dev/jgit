...
public abstract class PackIndexWriter {
     ...
     *
     * @param dst  the stream the index data will be written to. If not already
     *             buffered it will be automatically wrapped in a buffered
     *             stream. Callers are always responsible for closing the stream.
     * @param objs the objects the caller needs to store in the index. Entries
     ...
     * the supplied stream.
     * @throws IllegalArgumentException no recognized pack index version can support the supplied
     ...
     *                buffered it will be automatically wrapped in a buffered
     *                stream. Callers are always responsible for closing the stream.
     * @param version index format version number required by the caller. Exactly
     *                this formatted version will be written.
     * @return a new writer to output an index file of the requested format to
     * the supplied stream.
     * @throws IllegalArgumentException the version requested is not supported by this
     *                                  implementation.
     */
     ...
     *
     * @param dst the stream this instance outputs to. If not already buffered
     ...
     *
     * @param toStore          sorted list of objects to store in the index. The caller must
     *                         have previously sorted the list using
     *                         {@link PackedObjectInfo}'s native
     *                         {@link Comparable} implementation.
     * @param packDataChecksum checksum signature of the entire pack data content. This is
     *                         traditionally the last 20 bytes of the pack file's own stream.
     * @throws IOException an error occurred while writing to the output stream, or this
     ...
     * formatted index.
     *
     * @param version version number of this index format being written.
