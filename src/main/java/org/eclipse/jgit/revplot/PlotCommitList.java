 ...
 *
 * @param <L> type of lane used by the application.
 */
public class PlotCommitList<L extends PlotLane> extends
     ...
     * order.
     *
     * @param currCommit the commit the caller needs to get the lanes from.
     * @param result     collection to add the passing lanes into.
     */
    ...
    protected void enter(int index, PlotCommit<L> currCommit) {
        ...
        if (nChildren == 0) {
            ...
            @SuppressWarnings("unchecked") final PlotCommit<L> c = currCommit.children[0];
            ...
            for (int i = 0; i < nChildren; i++) {
                @SuppressWarnings("unchecked") final PlotCommit<L> c = currCommit.children[i];
     ...
     *
     * @param index       the index of <code>currCommit</code> in the list
     * @param currCommit  the current commit
     * @param childOnLane the direct child on the same lane as <code>currCommit</code>,
     ...
     * @param laneToContinue lane to continue
     */
    @SuppressWarnings("ReferenceEquality")
    ...
    /**
     * @param blockedPositions may be null
